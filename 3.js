...
const extractActions = (instructions) => instructions.split(',').map(i => ({ dir: i.substring(0,1), dist: parseInt(i.substring(1)) }))

const convertDirsToPath = (dirs) => dirs.reduce((path, step, i) => {
    const [curX, curY] = path[i]
    const [dX, dY] = action[step.dir](step.dist)
    return path.concat([[curX + dX, curY + dY]])
}, [[0, 0]])

const convertPathToLines = path => Array.from({ length: path.length - 1 }, (_, i) => ({ from: path[i], to: path[i+1] }))

const getDistanceFromActions = dirs => dirs.reduce((acc, { dist }, i) => acc.concat([acc[i] + Math.abs(dist)]), [0])

const wire1Dirs = extractActions(wire1)
const wire1Path = convertDirsToPath(wire1Dirs)
const wire1Lines = convertPathToLines(wire1Path)
const wire1Distance = getDistanceFromActions(wire1Dirs)

const wire2Dirs = extractActions(wire2)
const wire2Path = convertDirsToPath(wire2Dirs)
const wire2Lines = convertPathToLines(wire2Path)
const wire2Distance = getDistanceFromActions(wire2Dirs)
...
const getClosestIntersection = () => {
    ...
    wire1Lines.map((a, ai) =>
        wire2Lines.map((b, bi) => {
        const linesArePerpendicular = a.from[0] === a.to[0] && b.from[1] === b.to[1] || a.from[1] === a.to[1] && b.from[0] === b.to[0]

        if (linesArePerpendicular) {

            const aMinX = Math.min(a.from[0], a.to[0])
            const aMaxX = Math.max(a.from[0], a.to[0])
            const aMinY = Math.min(a.from[1], a.to[1])
            const aMaxY = Math.max(a.from[1], a.to[1])
            const bMinX = Math.min(b.from[0], b.to[0])
            const bMaxX = Math.max(b.from[0], b.to[0])
            const bMinY = Math.min(b.from[1], b.to[1])
            const bMaxY = Math.max(b.from[1], b.to[1])

            const intersect = aMaxX >= bMinX && aMinX <= bMaxX && aMinY <= bMaxY && aMaxY >= bMinY

            if (intersect) {
                if (aMinX === aMaxX) { // if match then aX is the intersection point
                    const additionalDistanceA = Math.abs(bMinY - a.from[1])
                    const additionalDistanceB = Math.abs(aMinX - b.from[0])
                    intersectionDistances.push(additionalDistanceA + additionalDistanceB + wire1Distance[ai] + wire2Distance[bi])
                } else {
                    const additionalDistanceA = Math.abs(bMinX - a.from[0])
                    const additionalDistanceB = Math.abs(aMinY - b.from[1])
                    intersectionDistances.push(additionalDistanceA + additionalDistanceB + wire1Distance[ai] + wire2Distance[bi])
                }
            }
        }
